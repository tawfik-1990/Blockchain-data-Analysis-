...
centr_degree(ig_all)$centralization
cent<-centr_degree(ig_all, mode = "all",
                   loops = TRUE, normalized = TRUE)
centeigen<-centralize(indegree_df_all$indegree, theoretical.max = 0, normalized = TRUE)
centbet<-centr_betw(ig_all, directed = FALSE)$centralization
#calculate in-degree
...
indegree1<- degree(ig_all,  mode = "all")
indegree_df1<-data.frame(Name=V(ig_all)$name,indegree=as.vector(indegree1))
indegree_df_all1<-indegree_df1[order(indegree_df1$indegree,decreasing = TRUE),]
...
centralize.scores (0.9, theoretical.max, normalized = TRUE)
tr<-centr_degree_tmax(graph = ig_all, nodes = 100, mode = "all", loops = FALSE)
ss<-centralize.scores (weighted_out_degree$weightedoutdegree, theoretical.max, normalized = TRUE)

devsio<-data.frame (num=eigen_centrality_sort$eigenC)
dev<-centr_eigen(ig_all, normalized = TRUE)$centralization
centralization<- function(df)
{
  x<-0
  for (i in 1:nrow(weighted_out_degree))
  {
    x<- (weighted_out_degree$weightedoutdegree[1] - weighted_out_degree$weightedoutdegree[i]) + x
    
  }
  
  centrat<-(x/((703047*703047)-(3*703047)+2))
}
...
# nodes_of_interest <- split(xy.df, seq(nrow(xy.df)))df<-data.frame(Adresse=character(),most_sender =character(),Number=numeric(),most_recipient=character(),
...
df<-data.frame(Adresse=character(),most_sender =character()
              , stringsAsFactors=FALSE)
for(i in 1:nrow(publickey)){ 
  print(i)
  
ego_all<-ego(ig_all, order = 1, nodes = which(V(ig_all)$name==publickey$adress[i]), mode =  "in", mindist = 0)
...
list_of_edges <-E(ig_all)[V(ig_all)[name==publickey$adress[i]] %<-% V(ig_all)[name %in% avector ]]
...
corr1<-Similarity_cor(balnced_adress,betweenness_Centrality_sort)
...
read<-read.csv("rf.csv")
min<-apply(read,2,max)
min1 <-data.frame(name= min)
